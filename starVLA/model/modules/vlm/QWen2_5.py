...
class _QWen_VL_Interface(nn.Module):
    ...
    def build_qwenvl_inputs(self, images, instructions, solutions=None, **kwargs):
        ...
        for imgs, instruction in zip(images, instructions):
            ...
            if prompt_cfg is not None and "CoT_prompt" in prompt_cfg:  # If using a grounding prompt to task
        ...
        # If solutions are provided, keep only assistant answer span in labels.
        if solutions is not None:
            labels = torch.full_like(batch_input["input_ids"], IGNORE_INDEX)
            prefix_ids = self.processor.tokenizer(
                "<|im_start|>assistant\n", add_special_tokens=False
            ).input_ids
            prefix = torch.tensor(prefix_ids, device=batch_input["input_ids"].device)
            ...
            if pad_id is not None:
                labels[batch_input["input_ids"] == pad_id] = IGNORE_INDEX
