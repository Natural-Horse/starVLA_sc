...
def _resolve_token_id(tokenizer: transformers.PreTrainedTokenizer, token: str, default: int) -> int:
    ...
    if token_id == unk_token_id and token not in tokenizer.get_vocab():
        return default
    return int(token_id)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args):
        ...
        self.rope_token_ids = {
            "vision_start_token_id": _resolve_token_id(
                tokenizer, "<|vision_start|>", VISION_START_TOKEN_INDEX
            ),
            "image_token_id": _resolve_token_id(tokenizer, "<|image_pad|>", IMAGE_TOKEN_INDEX),
            "video_token_id": _resolve_token_id(tokenizer, "<|video_pad|>", VIDEO_TOKEN_INDEX),
        }
    ...
    def resolve_media_path(self, media_folder: str, media_file: str) -> str:
        return os.path.join(media_folder, media_file)
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "images" in sources[0] and len(sources[0]["images"]):
            ...
            if isinstance(image_file, List):
                if len(image_file) > 1:
                    image_file = [self.resolve_media_path(image_folder, file) for file in image_file]
                    results = [self.process_image_unified(file) for file in image_file]
                    image, grid_thw = zip(*results)
                else:
                    image_file = image_file[0]
                    image_file = self.resolve_media_path(image_folder, image_file)
            ...
            if isinstance(video_file, List):
                if len(video_file) > 1:
                    video_file = [self.resolve_media_path(video_folder, file) for file in video_file]
                    results = [self.process_video(file) for file in video_file]
                    video, grid_thw, second_per_grid_ts = zip(*results)
                else:
                    video_file = video_file[0]
                    video_file = self.resolve_media_path(video_folder, video_file)
...
class VsiSmallLazySupervisedDataset(LazySupervisedDataset):
    """VSI-small SFT adapter.

    The JSON is standard ``images + conversations`` data. Only the YouTube
    room-tour image paths need a local fallback because the JSON contains
    ``ytb_roomtour/part_*/xxx.jpg`` while files are stored flat under
    ``ytb_roomtour/xxx.jpg``.
    """

    def resolve_media_path(self, media_folder: str, media_file: str) -> str:
        media_path = super().resolve_media_path(media_folder, media_file)
        if os.path.exists(media_path):
            return media_path

        parts = media_file.split("/")
        if len(parts) >= 3 and parts[0] == "ytb_roomtour" and parts[1].startswith("part_"):
            fallback_path = os.path.join(media_folder, parts[0], *parts[2:])
...
def make_supervised_data_module(
    ...
    train_dataset = dataset_cls(tokenizer=tokenizer, data_args=data_args)
    ...
    if hasattr(data_args, "eval_dataset") and data_args.eval_dataset:
        ...
        eval_dataset = dataset_cls(tokenizer=tokenizer, data_args=eval_data_args)
...
def make_vlm_dataloader(cfg):
    ...
    model_max_length = int(getattr(data_args, "model_max_length", 2048))
    max_pixels = int(getattr(data_args, "max_pixels", 50176))
    min_pixels = int(getattr(data_args, "min_pixels", 784))
    ...
    if not hasattr(data_args_ns, "data_flatten"):
        data_args_ns.data_flatten = False
    if not hasattr(data_args_ns, "model_type"):
        data_args_ns.model_type = "qwen2.5vl"
    data_args_ns.image_processor = image_processor  # TODO later remove the logic bound to model
    dataset_cls = (
        VsiSmallLazySupervisedDataset
        if bool(getattr(data_args_ns, "vsi_small_path_fallback", False))
        else LazySupervisedDataset
    )
    data_module = make_supervised_data_module(tokenizer=tokenizer, data_args=data_args_ns, dataset_cls=dataset_cls)
