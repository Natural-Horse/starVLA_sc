...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        def normalize_statistics_by_mode(raw_statistics):
            if any(k in raw_statistics for k in ["abs", "delta", "rel"]):
                normalized = {}
                for mode, stats in raw_statistics.items():
                    if mode not in ["abs", "delta", "rel"]:
                        continue
                    if isinstance(stats, dict) and "statistics" in stats and isinstance(stats["statistics"], dict):
                        normalized[mode] = stats["statistics"]
                    else:
                        normalized[mode] = stats
                return normalized
            if "statistics" in raw_statistics and isinstance(raw_statistics["statistics"], dict):
                return {"abs": raw_statistics["statistics"]}
            cleaned = {k: v for k, v in raw_statistics.items() if not str(k).startswith("__")}
            return {"abs": cleaned}

        stats_path = self.dataset_path / LE_ROBOT_STATS_FILENAME
        tmp_path = stats_path.with_suffix(".tmp")
        stats_dir_writable = os.access(stats_path.parent, os.W_OK)
        
        # ---------- all rank try to read  ----------
        if stats_path.exists():
            try:
                with open(stats_path, "r") as f:
                    le_statistics_by_mode = normalize_statistics_by_mode(json.load(f))
            except Exception as e:
                print(
        ...
        computed_any = False
        if is_main() or not stats_dir_writable:
            action_keys_full = []
            state_keys_full = []
            ...
            if computed_any and stats_dir_writable:
                ...
                print(
                    f"[RANK {os.environ.get('RANK', 'NA')}] "
                    f"Dataset statistics were computed in memory because {stats_path.parent} is not writable."
                )

        # ---------- sync ----------
        if dist.is_initialized() and stats_dir_writable:
            dist.barrier()
        
        # ---------- all rank read again ----------
        if stats_dir_writable and (not is_main() or computed_any):
            with open(stats_path, "r") as f:
                le_statistics_by_mode = normalize_statistics_by_mode(json.load(f))
