...
def _module_has_zero3_params(module):
        ...
        if reload_modules:  # partial load
            ...
            for path in module_paths:
                ...
                try:
                    ...
                    if sub_state_dict:
                        _load_state_dict_zero3_aware(module, sub_state_dict, strict=True)
                        if not dist.is_initialized() or dist.get_rank() == 0:
            ...
            try:
                _load_state_dict_zero3_aware(model, checkpoint, strict=False)
                if not dist.is_initialized() or dist.get_rank() == 0:
