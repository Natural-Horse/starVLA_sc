...
RUN_ID="${RUN_ID:-qwenpi_wallx_stage2_flow_from_fast_default_h24_bbox_detach_action_grad}"
...
FAST_TOKENIZER=/diff/wallx_workspace/wall-x/fast-tokenizer
STAGE1_CKPT=/diff/wallx_workspace/starVLA/results/Checkpoints/qwenpi_wallx_fast_default_h24_bbox/final_model/pytorch_model.pt

log() {
  printf '[%s] %s\n' "$(date '+%F %T')" "$*"
}

log "Sleeping ${DELAY_SECONDS}s before checking Stage1 default FAST checkpoint."
sleep "${DELAY_SECONDS}"
